...
void post_all(sender_t *sender) {
	while(1) {
		...
		zlog_debug(sender->tag, "Wait for %lums", NANO/2/1000000);
		snyo_sleep(NANO/2);
	}
}

void register_json(sender_t *sender, char *json) {
	sender->queue[sender->tail] = json;
	if((sender->tail+1)%MAX_LENGTH == sender->head) {
		zlog_info(sender->tag, "POST queue is full, flushing buffer");
	}
}